...
```lua
local coroutine_object = start_async(function(arg)
    print(arg)
end, "hello")
```
...
```lua
local address_a = "140702365450240"
local address_b = "0x7FF812345000"
```
...
```lua
local data = await_async(
    memory.async_read_rva(pid, 0x1234, 4)
)
```
...
function OnRender()
    ui.window("Kernel Script", function()
        state.process_name = select(
            1,
            ui.text_edit(state.process_name, false, false, false)
        )
